...
The models differ in circuit depth, entanglement topology, and output head: qml_3
is a single-circuit binary classifier (reps=3, all-to-all entanglement, 54 trainable
parameters); qml_5 is a two-stage binary-then-count regressor (reps=1,
ring entanglement, trained on a natural-distribution sample); and qml_6
uses the same two-stage architecture as qml_5 but trained on a class-balanced
...
AUC-ROC=0.873 on the 2022 test set. The best QML model (qml_3) achieves
AUC-ROC=0.667, trained on only 1,800 rows due to quantum simulator constraints.
We demonstrate that the performance gap is attributable primarily to the 52 times
